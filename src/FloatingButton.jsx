...
function Workspace() {
    ...
    const addWidget = (type) => {
        const id = Date.now();
        draggableRefs.current[id] = React.createRef(); // Create a ref for the new widget
            ...
            <div className="fixed bottom-10 right-10 flex flex-col items-center space-y-2">
                {widgets.map((widget) => (
                    <Draggable key={widget.id} nodeRef={draggableRefs.current[widget.id]}>
                        <div ref={draggableRefs.current[widget.id]} className="absolute p-4 bg-white shadow-lg rounded-lg">
                            <button
                ...
                {isOpen && (
                    <div className="flex flex-col items-center space-y-2 mb-2">
                        <button onClick={() => addWidget("counter")} className="bg-blue-600 p-3 rounded-full shadow-lg">
                            <PiClockCounterClockwiseBold className="text-white" />
                        </button>
                        <button onClick={() => addWidget("todolist")} className="bg-yellow-600 p-3 rounded-full shadow-lg">
                            <FaList className="text-white" />
                        </button>
                        <button onClick={() => addWidget("timer")} className="bg-red-600 p-3 rounded-full shadow-lg">
                            <FaStopwatch className="text-white" />
                        </button>
                        <button onClick={() => addWidget("notes")} className="bg-green-600 p-3 rounded-full shadow-lg">
                            <FaStickyNote className="text-white" />
                        </button>
                    </div>
                )}

                <button onClick={toggleMenu} className="bg-white p-4 rounded-full shadow-lg border border-gray-300">
